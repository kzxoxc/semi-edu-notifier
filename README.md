...
반도체 관련 교육 공고 6곳(한국반도체아카데미 / 반도체인프라활용현장인력양성 / 차세대반도체컨소시엄 /
인하대 취업프로그램 / 한국반도체교육원(KSTI) / 나노종합기술원)을
...
- **사이트 F(나노종합기술원)**: robots.txt가 전체 경로(`Disallow: /`)를 막고 있는데, 사이트 B와
  동일하게 사용자 확인 후 무시하고 수집하는 중. 상세보기도 JS 모달 방식이라 직접 링크가 없어서
  목록 페이지 링크로 대체한다.
